...
// processCollectionDomainEvent processes collection domain events
func (c *EventConsumer) processCollectionDomainEvent(ctx context.Context, delivery amqp.Delivery) error {
	// Parse the message body as domain event
	var domainEvent domain.DomainEvent
	err := json.Unmarshal(delivery.Body, &domainEvent)
	if err != nil {
		return fmt.Errorf("failed to unmarshal domain event: %w", err)
	}

	// Validate event
	if err := c.validateDomainEvent(&domainEvent); err != nil {
		return fmt.Errorf("invalid domain event: %w", err)
	}

	// Extract chain ID from routing key if not present in event
	if domainEvent.ChainID == "" {
		domainEvent.ChainID = c.extractChainIDFromRoutingKey(delivery.RoutingKey)
	}

	// Add message metadata
	if domainEvent.EventID == "" {
		domainEvent.EventID = delivery.MessageId
	}

	// Set timestamp if not present
	if domainEvent.Timestamp.IsZero() {
		if !delivery.Timestamp.IsZero() {
			domainEvent.Timestamp = delivery.Timestamp
		} else {
			domainEvent.Timestamp = time.Now()
		}
	}
	...
	log.Printf("Processing collection domain event: EventID=%s, Type=%s, AggregateID=%s, ChainID=%s",
		domainEvent.EventID, domainEvent.EventType, domainEvent.AggregateID, domainEvent.ChainID)

	return handler(ctx, &domainEvent)
}

// validateDomainEvent validates the domain event structure
func (c *EventConsumer) validateDomainEvent(event *domain.DomainEvent) error {
	if event.EventType == "" {
	...
	case "collection_upserted", "collection_created":
		requiredFields := []string{"contract_address", "creator", "name"}
...
func (c *EventConsumer) getEventTypeFromRoutingKey(routingKey string) string {
	// Expected format: collections.domain.upserted.eip155-1
	parts := strings.Split(routingKey, ".")
	if len(parts) >= 3 {
		eventType := parts[2] // "upserted"
		if eventType == "upserted" {
			return "collection_upserted"
		}
		return "collection_" + eventType
	}
	return "unknown"
}
